...
```console
WRN The toolchain running the scan carries advisories the declared version does not advisories="CVE-2026-39822, CVE-2026-42505" declared=1.25.12 toolchain=go1.26.4
```

Still worth acting on: binaries built on that machine carry the advisory until its toolchain moves. The version named is the `go` on PATH, which is what the scan analysed, rather than the one this binary was built with. An advisory the declaration sits below stays on the row, offered the lowest fix above it, since a `go` directive is a floor an affected toolchain can honour. So does one the database says nothing about.
