...
// TestLoadPerModuleCooldown checks that a pattern can set the period its own modules
// must settle for, which is what lets a project take its own releases at once.
func TestLoadPerModuleCooldown(t *testing.T) {
	path := write(t, t.TempDir(), "policy.json", `{
      "cooldown": "7d",
      "actions":  {"warn": {"exit": 0}},
      "modules": {
        "github.com/opensearch-project/**": {"cooldown": "0"},
        "example.com/slow":                 {"cooldown": "30d"},
        "example.com/plain":                {"allow": "*"}
      },
      "rules": [{"when": "archived", "then": "warn"}]
    }`)
	p, err := Load([]string{path})
	...
	for _, tc := range []struct {
		...
		// The case this exists for: a wildcard covering everything one project publishes.
		{module: "github.com/opensearch-project/opensearch-go/v5", want: 0, found: true},
		{module: "github.com/opensearch-project/opensearch-go/v5/osprom", want: 0, found: true},
		// A period may also be lengthened, since the field says which one governs.
		{module: "example.com/slow", want: 30 * 24 * time.Hour, found: true},
		// A pattern that says nothing about the period leaves the run's to decide,
		// which is distinct from asking for zero.
		{module: "example.com/plain", found: false},
		// So does a module no pattern covers at all.
		{module: "example.com/unmatched", found: false},
	} {
		got, ok := p.ModuleCooldown(tc.module)
		if ok != tc.found {
			t.Errorf("ModuleCooldown(%q) found = %v, want %v", tc.module, ok, tc.found)
			continue
		}
		if ok && got != tc.want {
			t.Errorf("ModuleCooldown(%q) = %v, want %v", tc.module, got, tc.want)
		}
	}
}

// TestLoadPerModuleCooldownPrefersTheSpecificPattern checks that the most specific
// pattern decides, as it does for permission: a rule on an exact path outranks one
// covering the whole host it sits under.
func TestLoadPerModuleCooldownPrefersTheSpecificPattern(t *testing.T) {
	path := write(t, t.TempDir(), "policy.json", `{
      "actions": {"warn": {"exit": 0}},
      "modules": {
        "github.com/acme/**":     {"cooldown": "0"},
        "github.com/acme/risky":  {"cooldown": "14d"}
      },
      "rules": [{"when": "archived", "then": "warn"}]
    }`)
	p, err := Load([]string{path})
	...
	if got, _ := p.ModuleCooldown("github.com/acme/ordinary"); got != 0 {
		t.Errorf("ModuleCooldown(ordinary) = %v, want the wildcard's zero", got)
	}
	if got, _ := p.ModuleCooldown("github.com/acme/risky"); got != 14*24*time.Hour {
		t.Errorf("ModuleCooldown(risky) = %v, want the exact pattern's 14d", got)
	}
}

// TestLoadPerModuleCooldownStacks checks that a period set for a pattern survives a
// later file restating that pattern for another reason.
//
// The same per-field merge the rest of a mark uses: an allow-list regenerated by
// --format=policy names every module it permits, and must not silently drop the period
// a baseline set.
func TestLoadPerModuleCooldownStacks(t *testing.T) {
	dir := t.TempDir()
	base := write(t, dir, "baseline.json", `{
      "actions": {"warn": {"exit": 0}},
      "modules": {"github.com/acme/lib": {"cooldown": "0"}},
      "rules":   [{"when": "archived", "then": "warn"}]
    }`)
	generated := write(t, dir, "allow-list.json", `{
      "modules": {"github.com/acme/lib": {"allow": "go.mod"}}
    }`)

	p, err := Load([]string{base, generated})
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	got, ok := p.ModuleCooldown("github.com/acme/lib")
	if !ok {
		t.Fatal("the period was lost behind the regenerated allow-list")
	}
	if got != 0 {
		t.Errorf("ModuleCooldown = %v, want the baseline's zero", got)
	}
	// The generated permission still applies.
	if d := p.Check("github.com/acme/lib", ver(t, "v1.0.0"), ver(t, "v1.0.0")); d.Verdict != Allowed {
		t.Errorf("verdict = %s, want the allow-list's permission to stand", d.Verdict)
	}
}

// TestLoadRejectsBadModuleCooldown checks that a period that will not parse fails while
// the file is being read, rather than being mistaken for no period at all.
func TestLoadRejectsBadModuleCooldown(t *testing.T) {
	path := write(t, t.TempDir(), "policy.json", `{
      "actions": {"warn": {"exit": 0}},
      "modules": {"example.com/x": {"cooldown": "next tuesday"}},
      "rules":   [{"when": "archived", "then": "warn"}]
    }`)
	_, err := Load([]string{path})
	if err == nil {
		t.Fatal("Load accepted an unparseable period, want an error")
	}
