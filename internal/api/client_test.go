...
func TestFetchModuleVersions_NotFoundIsCachedAsEmpty(t *testing.T) {
	t.Setenv("GOMODUPGRADE_CACHE_DIR", t.TempDir())
	...
	items, err := client.FetchModuleVersions(context.Background(), "github.com/private/repo")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(items) != 0 {
	...
	if _, err := client.FetchModuleVersions(context.Background(), "github.com/private/repo"); err != nil {
		t.Fatalf("unexpected error on second call: %v", err)
	}
	if requests != 1 {
		t.Errorf("expected 404 to be negatively cached (1 request), got %d", requests)
	}
}
...
func TestRetryDelay(t *testing.T) {
	...
	if d := retryDelay(h, 0); d != 30*time.Second {
		t.Errorf("Retry-After must be capped at 30s, got %v", d)
	}
	if d := retryDelay(http.Header{}, 0); d != 2*time.Second {
		t.Errorf("expected 2s backoff on attempt 0, got %v", d)
	}
	if d := retryDelay(http.Header{}, 1); d != 4*time.Second {
		t.Errorf("expected 4s backoff on attempt 1, got %v", d)
	}
}
