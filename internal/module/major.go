...
func FindMajorUpgrades(currentPath string, currentVerStr string, items []api.VersionItem) ([]Module, error) {
	...
	for _, item := range items {
		...
		candidate := majorVersionPath(currentPath, major)
		// Same-path upgrades (v0 -> v1) are already reported by `go list -u`
		// and must not go through the rewrite + `go get old@none` flow.
		if candidate == currentPath || item.ModulePath != candidate {
			continue
		}
